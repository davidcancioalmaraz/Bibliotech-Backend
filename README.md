...
src/
  ...
  user/
    ...
    password.ts           bcrypt hashing helper, shared with the seeder
    dto/                  Request payloads
...
The role is the `UserRole` enum exported from the same entity file, materialised
in Postgres as `user_role_enum`. Passwords are hashed with bcrypt through
`hashPassword()` in `src/user/password.ts`, the single place where the cost
factor is defined; both `UserService.create` and the seeder go through it.
...
`user.seeder.ts` has no factory on purpose: the three users are fixed rows —
one `admin` and two `member` — so that there are stable credentials to log in
with after every reseed. They all share the same password, `Bibliotech123`
...
- `SEED_FRESH=true` — `TRUNCATE ... RESTART IDENTITY CASCADE` on `loan`, `book`
  and `user` before inserting, so the tables are rebuilt from scratch with ids
  starting at 1.
- `SEED_USER_PASSWORD=<password>` — the password given to the three seeded users
  (default `Bibliotech123`).
